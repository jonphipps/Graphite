...
class Graphite
{
    ...
    public function __construct($namespaces = array(), $uri = null)
    {
        ...
        $this->labelRelations = array("skos:prefLabel",
                                      "rdfs:label",
                                      "foaf:name",
                                      "dct:title",
                                      "dc:title",
...
';
    ...
    /**
     * $dir should be a directory the web server has permission to read and write to. Any RDF/XML documents which graphite downloads will be saved here. If a cache exists and is newer than $age seconds then load() will use the document in the cache directory in preference to doing an HTTP request. $age defaults to 24*60*60 - 24 hours. This including this function can seriously improve graphite performance! If you want to always load certain documents, load them before setting the cache.
     *
    ...
    /**
     * Return a list of the relations currently used for $resource->label(), if called with a parameter then this should be an array to <strong>replace</strong> the current list. To just add additional relation types to use as labels, use addLabelRelation($relation).
     */
